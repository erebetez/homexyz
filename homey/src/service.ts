...
async function storeDevice(client, device) {
    const retr = await client.query("SELECT status FROM devices WHERE id = $1", [device.id]);
    let text: string;

    if (retr.rowCount > 0) {
        // Existing device
        text = 'UPDATE devices SET name = $2, description = $3, ip = $4, status = $5 WHERE id = $1';
    } else {
        // New device
        text = 'INSERT INTO devices( id, name, description, ip, status ) VALUES($1, $2, $3, $4, $5)';
    }
...
async function storeStates(client, device) {
    const retr = await client.query("SELECT key FROM states WHERE id = $1", [device.id]);
    ...
    for (let key in device.states) {
        ...
        if (existingKeys.includes(key)) {
            text = 'UPDATE states SET id = $2, updated = NOW(), attribute = $3 WHERE key = $1';
        } else {
            text = 'INSERT INTO states( key, id, inserted, updated, attribute ) VALUES($1, $2, NOW(), NOW(), $3)';
        }
...
async function storeEvent(event) {
    const text = 'INSERT INTO events(key, inserted, value, trail) VALUES($1, NOW(), $2, $3)';
    const values = [event.key, JSON.stringify(event.value), JSON.stringify(event.trail)];
...
async function getDevice(selector, where, cb) {
    const text = "SELECT d.id, d.name, d.description, d.ip, d.status, s.key, s.attribute FROM devices d LEFT JOIN states s ON d.id = s.id WHERE " + where + " = $1";
...
export { storeEvent, storeIot, getDevices, updateDeviceState, getStates, getDeviceByKey, getDeviceById, getEvents, getLogs };